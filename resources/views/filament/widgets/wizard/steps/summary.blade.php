...
<div class="wizard-summary-parity" x-data="{}">
    ...
    <section class="it-page-section mb-5" id="summary-section-cosa">
        <div class="d-flex justify-content-between align-items-center border-bottom pb-2 mb-3">
            <h3 class="h5 mb-0 fw-bold text-uppercase text-primary">
                {{ __('fixcity::segnalazione.sections.summary_cosa.label') ?? '1. Cosa' }}
            </h3>
            <a href="javascript:void(0)" class="btn btn-link btn-sm d-flex align-items-center gap-1" 
               wire:click="goToStep('data')">
                ...
                <span>{{ __('fixcity::segnalazione.actions.edit.label') ?? 'Modifica' }}</span>
            </a>
        </div>
        <dl class="row g-0">
            <dt class="col-sm-3 fw-semibold text-muted small uppercase">{{ __('fixcity::segnalazione.fields.type.label') ?? 'Categoria' }}</dt>
    ...
    <section class="it-page-section mb-5" id="summary-section-dove">
        <div class="d-flex justify-content-between align-items-center border-bottom pb-2 mb-3">
            <h3 class="h5 mb-0 fw-bold text-uppercase text-primary">
                {{ __('fixcity::segnalazione.sections.summary_dove.label') ?? '2. Dove' }}
            </h3>
            <a href="javascript:void(0)" class="btn btn-link btn-sm d-flex align-items-center gap-1" 
               wire:click="goToStep('data')">
                <svg class="icon icon-xs"><use href="/themes/Sixteen/design-comuni/assets/bootstrap-italia/dist/svg/sprites.svg#it-pencil"></use></svg>
        ...
        <dl class="row g-0">
            <dt class="col-sm-3 fw-semibold text-muted small uppercase">{{ __('fixcity::segnalazione.fields.address.label') ?? 'Indirizzo' }}</dt>
        ...
        <div class="map-preview-container mt-3 rounded overflow-hidden border" style="height: 200px;">
            {{-- Inserire qui un'anteprima statica o readonly della mappa --}}
            <div class="bg-light d-flex align-items-center justify-content-center h-100 text-muted small">
                {{-- Mock per ora, integrabile con un componente Leaflet non interattivo --}}
    ...
    <section class="it-page-section mb-5" id="summary-section-dettagli">
        <div class="d-flex justify-content-between align-items-center border-bottom pb-2 mb-3">
            <h3 class="h5 mb-0 fw-bold text-uppercase text-primary">
                {{ __('fixcity::segnalazione.sections.summary_dettagli.label') ?? '3. Dettagli' }}
            </h3>
            <a href="javascript:void(0)" class="btn btn-link btn-sm d-flex align-items-center gap-1" 
               wire:click="goToStep('data')">
                <svg class="icon icon-xs"><use href="/themes/Sixteen/design-comuni/assets/bootstrap-italia/dist/svg/sprites.svg#it-pencil"></use></svg>
        ...
        <dl class="row g-0">
            <dt class="col-sm-3 fw-semibold text-muted small uppercase">{{ __('fixcity::segnalazione.fields.title.label') ?? 'Oggetto' }}</dt>
            <dd class="col-sm-9 text-dark fw-bold">{{ $formData['name'] ?? '' }}</dd>
            
            <dt class="col-sm-3 fw-semibold text-muted small uppercase mt-2">{{ __('fixcity::segnalazione.fields.content.label') ?? 'Descrizione' }}</dt>
            <dd class="col-sm-9 text-dark mt-2">{{ $formData['content'] ?? '' }}</dd>

            @if(count($images) > 0)
            <dt class="col-sm-3 fw-semibold text-muted small uppercase mt-3">{{ __('fixcity::segnalazione.fields.images.label') ?? 'Allegati' }}</dt>
    ...
    <section class="it-page-section mb-4" id="summary-section-segnalatore">
        <div class="d-flex justify-content-between align-items-center border-bottom pb-2 mb-3">
            <h3 class="h5 mb-0 fw-bold text-uppercase text-primary">
                {{ __('fixcity::segnalazione.sections.summary_author.label') ?? '4. Segnalatore' }}
            </h3>
            <a href="javascript:void(0)" class="btn btn-link btn-sm d-flex align-items-center gap-1" 
               wire:click="goToStep('data')">
                <svg class="icon icon-xs"><use href="/themes/Sixteen/design-comuni/assets/bootstrap-italia/dist/svg/sprites.svg#it-pencil"></use></svg>
        ...
        <dl class="row g-0">
            <dt class="col-sm-3 fw-semibold text-muted small uppercase">{{ __('fixcity::segnalazione.fields.author_name.label') ?? 'Nome e Cognome' }}</dt>
            <dd class="col-sm-9 text-dark">{{ $this->getAuthUserName() }}</dd>
            
            <dt class="col-sm-3 fw-semibold text-muted small uppercase mt-2">{{ __('fixcity::segnalazione.fields.email.label') ?? 'Email per contatti' }}</dt>
