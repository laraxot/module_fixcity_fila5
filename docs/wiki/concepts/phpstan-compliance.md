---
title: "Fixcity Module - PHPStan Type Compliance"
type: concept
...
```bash
vendor/bin/phpstan analyse laravel/Modules/Fixcity \
  --level=max \
  --no-progress
```
...
```bash
vendor/bin/phpstan analyse laravel/Modules/Fixcity --level=max
```
...
# Verbose mode
vendor/bin/phpstan analyse laravel/Modules/Fixcity --level=max -v
```
...
```bash
vendor/bin/pest laravel/Modules/Fixcity/tests --parallel
```
